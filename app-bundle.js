...
window.renderDashboard = async function() {
    ...
    if (!user) {
        ...
        <div class="bg-gradient-to-r from-blue-600 to-violet-600 rounded-3xl p-7 mb-8 text-white relative overflow-hidden">
            ...
            <div class="absolute -left-6 -bottom-6 w-32 h-32 bg-white/5 rounded-full blur-2xl pointer-events-none"></div>
            <div class="relative">
    ...
    container.innerHTML = `
    <div class="mb-8" id="dashboard-inner">
        <!-- Header -->
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6" id="dash-stats">
            ...
            <div class="bg-white rounded-2xl p-4 border border-gray-100 animate-pulse">
                <div class="w-8 h-8 bg-gray-100 rounded-xl mb-3"></div>
                <div class="w-14 h-5 bg-gray-100 rounded mb-1.5"></div>
                <div class="w-20 h-3 bg-gray-50 rounded"></div>
            </div>`).join('')}
        </div>
    ...
    const recentHtml = stats.recent?.length
        ? `<div class="space-y-2">
            ${stats.recent.map(inv => {
                ...
                return `
                <div class="flex items-center justify-between gap-2 px-3 py-2.5 rounded-xl hover:bg-blue-50 transition-all cursor-pointer group"
        ...
        : `<div class="flex flex-col items-center justify-center py-8 text-center">
            <div class="w-12 h-12 bg-gray-50 rounded-2xl flex items-center justify-center mb-3">
                <svg class="w-6 h-6 text-gray-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 13h6m-3-3v6m5 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
            </div>
        ...
        ? catEntries.map(([cat, count]) => {
            ...
            <div class="mb-3">
                <div class="flex items-center justify-between mb-1">
                    <span class="text-sm font-medium text-gray-700 flex items-center gap-1.5">${catEmoji(cat)} <span class="capitalize">${cat}</span></span>
                    <span class="text-xs font-bold text-gray-500">${count} <span class="text-gray-300">(${pct}%)</span></span>
                </div>
                <div class="h-2 bg-gray-100 rounded-full overflow-hidden">
                    <div class="h-full ${catColors[cat]||'bg-blue-400'} rounded-full transition-all" style="width:${pct}%"></div>
                </div>
            </div>`;
          }).join('')
